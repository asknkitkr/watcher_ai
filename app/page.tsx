...
import { CameraIcon, MoonIcon, PersonIcon, SpeakerLoudIcon, SunIcon, VideoIcon, ViewVerticalIcon } from '@radix-ui/react-icons';
...
const HomePage = (props: Props) => {
  ...
  function userPromptScreenshot() {
    if(!webcamRef.current) {
      toast('Camera not detected. Please refresh!')
    } else {
  ...
  function RenderFeatureHighlightsSection() {
    ...
    <ul className="space-y-4">
      ...
      <li>
        ...
        >
          {autoRecordEnabled ? <Rings color='white' height={30} /> : <PersonIcon/>}
  ...
  return (
    <div className='flex h-screen'>
        ...
        <div className="border-primary/5 border-2 max-w-xs flex flex-col gap-2 justify-between shadow-md rounded-md p-4">
          ...
          <Separator className='my-2'/>
              ...
              <Button variant={autoRecordEnabled ? 'destructive' : 'outline'}
                size={'icon'}
                onClick={toggleAutoRecord}>
                    {autoRecordEnabled ? <Rings color="white" height={45} /> : <PersonIcon/>}
                </Button>
          </div>
          {/* bottom section */}
          <div className="flex flex-col gap-2">
      ...
      {loading && <div className='z-50 absolute w-full h-full flex items-center justify-center bg-primary-foreground'>
        <span className='text-3xl'>Getting things ready...</span>
        </div>}
    </div>
  )
}
